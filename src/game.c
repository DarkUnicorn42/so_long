...
void	init_game(t_game *game)
{
	...
	game->img_floor = mlx_xpm_file_to_image(game->mlx, "./assets/bg.xpm", &img_width, &img_height);
	if (!game->img_floor)
		error_exit("Error loading floor texture");
	game->img_collect = mlx_xpm_file_to_image(game->mlx, "./assets/coin.xpm", &img_width, &img_height);
	if (!game->img_collect)
